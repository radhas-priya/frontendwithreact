...
const Volunteerform=() =>{
    ...
    const handleSubmit = async (e) => {
      ...
      const response = await fetch("http://localhost:5000/api/auth/volunteeruser", {
        method:'POST',
        headers:{
          'Content-Type': "application/json"
        },
        body:JSON.stringify({ name:credentials.name, email:credentials.email, password:credentials.password, age:credentials.age,city:credentials.city,state:credentials.state,zip:credentials.zip,phonennumber:credentials.phonenumber})
      });
  ...
  return (
    <>
      ...
      <center>
        ...
        <h2> Want to Do work on farms as volunteer? Fill the form down below</h2>
